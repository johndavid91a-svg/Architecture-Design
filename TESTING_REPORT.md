...
|---|---|---|
| Unit and integration tests | `npm test` | **160 passed**, 17 files, 0 failed |
...
| 21 | **Every PDF page was imported as a storey.** A drawing set is not a stack of floors: this 56-sheet set holds nine floor plans, and the rest are the title sheet, an index, a 3D view, a site plan, eight structural grids, eight opening plans, eight area blocks, two elevations, four sections and details. Importing one floor per page turned a nine-storey building into a 52-storey tower stacked at 11 ft intervals — a complete, confident, wrong answer, which is worse than an error. | Reported by the user, who knew their own building: basement, ground, mezzanine, four floors, mumty. | Added `sheet.ts`: sheets are classified by title into families, one family is chosen (working plans, because they carry room names), and each chosen sheet is mapped to a storey. `level` comes from the storey, never from the page index. The set now imports as 9 correctly named and ordered floors. |
