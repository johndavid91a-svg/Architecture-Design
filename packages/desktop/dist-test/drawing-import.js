...
export async function importDrawingFile(path) {
    ...
    const format = extension === 'pdf' ? 'pdf' : extension === 'dxf' ? 'dxf' : extension === 'ifc' ? 'ifc' : null;
    if (format === null) {
        return fail('ifc', path, extension === ''
    ...
    try {
        ...
        if (info.size > MAX_BYTES) {
            return fail(format, path, `That file is ${(info.size / 1_000_000).toFixed(0)} MB, above the ${MAX_BYTES / 1_000_000} MB limit.`);
        }
    }
    catch (error) {
        return fail(format, path, `Could not read the file: ${error.message}`);
    }
    switch (format) {
        case 'ifc':
...
async function importIfc(path) {
    try {
        ...
        return fail('ifc', path, `IFC import failed: ${error.message}`);
    }
}
async function importDxf(path) {
    try {
        const core = await import('@adp/core');
        ...
        return fail('dxf', path, `DXF import failed: ${error.message}`);
    }
}
async function importPdf(path) {
    try {
                    ...
                    : [
                        {
                            ...
                            remedy: 'The file is probably a scan. Export the drawing from CAD as a vector PDF, or import the DXF or IFC instead.',
                        },
                    ],
                stats: null,
            };
            ...
            return {
                ...
                issues: [
                    ...issues,
                    {
                        ...
                        message: `${pages.length} page(s) of vector line work were read, but a PDF page carries no ` +
                            `building scale.`,
                        remedy: 'Calibrate by picking two points on the drawing whose real distance you know, then import again.',
                    },
                ],
                stats: null,
        ...
        const floorSheets = chosen.floors.map((floor) => {
            ...
            if (!better || better.rooms < 2 || better.rooms <= own)
                return floor;
            substituted.push(`${floor.storey} from the ${better.sheet.family} plan on page ${better.sheet.pageNumber} ` +
        ...
        if (substituted.length > 0) {
            allIssues.push({
                ...
                message: `${substituted.length} storey(s) were read from a different sheet of the same storey, ` +
        ...
        if (chosen.floors.length > 0) {
            ...
            allIssues.push({
                ...
                message: `${pages.length} sheet(s) read; ${chosen.floors.length} of them are floor plans ` +
            ...
            allIssues.push({
                ...
                message: `None of the ${pages.length} sheet(s) could be identified as a floor plan from its title, ` +
                    `so no storey could be built.`,
                remedy: 'Sheet titles are read to tell a floor plan from an elevation or a section. If this set ' +
        ...
        if (stated) {
            ...
            allIssues.push({
                ...
                message: `The drawing states its own covered areas: ${summary}` +
            ...
            const comparison = core.compareWithDrawing(stated, floors.map((f) => ({
                storey: f.name,
            ...
            if (wrong.length > 0) {
                ...
                allIssues.push({
                    ...
                    message: `The room areas read from this PDF do not match the areas the drawing states. ` +
                        wrong
                            .map((c) => `${c.storey}: recognised ${c.recognisedSqft.toFixed(0)} sq ft against a stated ` +
                    ...
                    remedy: 'Import the DXF or IFC for measurable geometry. A PDF has no layers, so a dense plan ' +
        ...
        if (scaleless.length > 0) {
            allIssues.push({
                ...
                message: `${scaleless.length} floor plan(s) had no single usable scale and were left out: ` +
        ...
        return fail('pdf', path, `PDF import failed: ${error.message}`);
    }
}
function fail(format, filename, message) {
    return {
        ok: false,
        format,
        filename,
...
export async function listDrawingSheets(path) {
    const core = await import('@adp/core');
    const data = new Uint8Array(await readFile(path));
...
export async function readDrawingSheet(path, pageNumber) {
    const empty = {
        pageNumber,
    ...
    try {
        ...
        if (!page)
            return { ...empty, error: `Page ${pageNumber} carries no vector line work.` };
        const texts = core.recoverPageTexts(page.texts);
